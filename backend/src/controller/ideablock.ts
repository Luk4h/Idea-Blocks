...
export const newBlock = async (req: Request, res: Response) => {
  try {
    ...
    if (blockTitle && blockDescription) {
      ...
      res
        .status(200)
        .json('You new block ' + blockTitle + ' has been created!');
...
export const editBlock = async (req: Request, res: Response) => {
  try {
    const {
      ...
      blockTitle,
      blockDescription,
    }: {blockId: number; blockTitle: string; blockDescription: string} =
      req.body;
    if (String(blockId) && blockTitle && blockDescription) {
      await dbBlockIdeas.renameBlockIdea({
        blockId,
        blockTitle,
        blockDescription,
      });
...
export const deleteBlock = async (req: Request, res: Response) => {
  try {
    const {blockId, blockTitle}: {blockId: number; blockTitle: string} =
      req.body;
